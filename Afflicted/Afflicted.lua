...
function Afflicted:OnInitialize()
	self.defaults = {
		profile = {
			...
			spellVersion = 0,
			inside = {["arena"] = true, ["pvp"] = true, ["none"] = true},
			anchors = {},
			spells = {},
			arenas = {[2] = {}, [3] = {}, [5] = {}},
			anchorDefault = {
	...
	self.defaults.profile.anchors.interrupts.text = L["Interrupts"]
	...
	self.defaults.profile.anchors.defenses.text = L["Defensive"]
	self.defaults.profile.anchors.damage = CopyTable(anchor)
	self.defaults.profile.anchors.damage.text = L["Damage"]
	...
	elseif( self.db.profile.spellVersion < AfflictedSpells.version ) then
		...
		for spellID, data in pairs(spells) do
			...
			elseif(type(self.spells[spellID]) == "number") then
				self.db.profile.spells[spellID] = nil;
...
function Afflicted:UNIT_SPELLCAST_SUCCEEDED(event, unitTarget, castGUID, spellID)
	if (spellID == 42292) then
		if (not UnitIsEnemy("player", unitTarget)) then
            return false;
        end
...
function Afflicted:COMBAT_LOG_EVENT_UNFILTERED(event)
	...
	if (eventType == "SPELL_AURA_REMOVED" and (not self.db.profile.hostileOnly or sourceEnemy)) then
		...
		self:AbilityEarlyFade(sourceGUID, sourceName, spell, spellID, spellName)
		end

	-- Spell casted succesfully
	elseif (eventType == "SPELL_CAST_SUCCESS" and (not self.db.profile.hostileOnly or sourceEnemy)) then
		local spell = self:GetSpell(spellID, spellName)
		if( spell and spell.resets ) then
			self:ResetCooldowns(sourceGUID, spell.resets, spell)
		end
	...
	elseif (eventType == "SPELL_CREATE" and (not self.db.profile.hostileOnly or sourceEnemy)) then
		local spell = self:GetSpell(spellID, spellName)
	...
	elseif ((eventType == "PARTY_KILL" or (instanceType ~= "arena" and eventType == "UNIT_DIED")) and (not self.db.profile.hostileOnly or destEnemy)) then
		-- If this is a summoned object (trap/totem) that was specifically killed, remove its timer
		if( summonedObjects[destGUID] ) then
			self.bars:RemoveTimerByID(summonedObjects[destGUID])
			self.icons:RemoveTimerByID(summonedObjects[destGUID])
...
function Afflicted:SaveArenaBracket()
	...
	for i=1, maxBattlefieldID do
		local status, _, _, _, _, teamSize, _ = GetBattlefieldStatus(i)
		if( status == "active" and teamSize == "ARENA" ) then
			--local numOpponents = GetNumArenaOpponents()
			--arenaBracket = numOpponents
			--print("Hit arenabracket save"..arenaBracket)
			arenaBracket = 3
			
			break
		else
			--print ("didn't hit arenabracket save");
		end
	end
end
